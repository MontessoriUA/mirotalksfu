...
const MontemeetRoles = (() => {
    ...
    // Нажатие по пустому месту внизу экрана поднимает скрытый тулбар — и это же
    // нажатие успевает попасть по кнопке, которая только что под курсором (или
    // под пальцем) появилась. Запрет нажатий на скрытой панели снимался опросом
    // раз в четверть секунды, а между нажатием и кликом проходит меньше — в эту
    // щель оно и проскакивало (Иван, 2026-08-14). Смотрим на состояние панели в
    // момент нажатия и глушим ровно тот клик, который её и вызвал. Правило общее
    // для мыши и пальца: и там, и там панель появляется под указателем.
    function barHidden() {
        const bar = document.getElementById('bottomButtons');
        if (!bar) return true;
    ...
    document.addEventListener(
        ...
        (e) => {
            ...
            // человек в неё не целился. Анимацию мы сняли совсем, но условие
            // оставляем — вернётся плавность, вернётся и защита.
    ...
    // переподключиться сами (Иван, 2026-08-09), а заодно поднять камеру.
